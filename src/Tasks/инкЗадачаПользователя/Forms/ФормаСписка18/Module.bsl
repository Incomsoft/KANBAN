...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)        
	...
	Проект = ПолучитьПроектПоУмолчанию(Исполнитель);   
	
КонецПроцедуры              

&НаСервере
Функция ПолучитьПроектПоУмолчанию(Исполнитель)
	
	ПроектСсылка = Исполнитель.инкПроектПоУмолчанию;
...
Процедура УстановитьОтоборПоПроектуВСпике(ИмяСписка,ПроектСсылка)
	...
	ЭлементОтбораПользовательский.Использование = ЗначениеЗаполнено(ПроектСсылка);                            
