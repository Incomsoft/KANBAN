...
Функция ПолноеИмяФайлаВТоме(СвойстваФайла, ДатаДляРазмещенияВТоме = Неопределено) Экспорт
	...
	// +++ Чесноков М.С. 2023-02-09 0006
	Если ТипЗнч(СвойстваФайла.ВладелецФайла) = Тип("ЗадачаСсылка.инкЗадачаПользователя") Тогда
		КорневойКаталог = КорневойКаталог + "tasks" + Разделитель;	
	// --- Чесноков М.С. 2023-02-09 0006
	Иначе
		КорневойКаталог = КорневойКаталог + Формат(ДатаРазмещения, "ДФ=ггггММдд") + Разделитель;	
	КонецЕсли;
	// --- Чесноков М.С. 2023-01-30 0004
...
//
Функция СвободныйТом(РазмерФайла) Экспорт
